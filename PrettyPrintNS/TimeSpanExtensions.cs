...
using System.Linq;
using PrettyPrintNS.InternalTypes;
...
namespace PrettyPrintNS
{
    ...
    public static class TimeSpanExtensions
    {
            ...
            IFormatProvider formatProvider = null)
        {
            ...
            if (!Formats.TryGetValue(rep, out var timeFormat))
            ...
            switch (unit)
            {
                ...
                default:
                    throw new ArgumentException(
                        $"Flag not supported [{unit}]. Note that flag must not be a combination of multiple flags.");
            }
        }
        ...
        private static string GetCompactUnitString(TimeSpanUnit unit)
        {
            return unit switch
            {
                ...
                TimeSpanUnit.Milliseconds => "ms",
                _ => throw new NotImplementedException("TimeSpanUnit: " + unit)
            };
        }
